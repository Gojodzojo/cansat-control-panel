...
export const App: FC<props> = ({defautlUtilities}) => {
  ...
  const utilities = useMemo(() => openedUtilities.map(({utilityName, id}) => {
    const props = {
      removeUtility: () => setOpenedUtilities(utils => utils.filter((u) => u.id !== id) ),
      bigWindow: openedUtilities.length === 1,
      key: id
    } 
    if(utilityName === "Data table") {
      return <DataTable {...props} />  
    }
    else if(utilityName === "Visualizer") {
      return <UnityVisualiser {...props}/>
    }
    else if(utilityName === "Graph") {
      return <Graph {...props} />
    }
    else if(utilityName === "Message sender") {
